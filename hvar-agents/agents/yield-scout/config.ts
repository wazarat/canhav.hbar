...
export function getYieldScoutCounterpartyConfig(): CounterpartyConfig {
  ...
  const minReputation = Number(process.env.HVAR_MIN_REPUTATION ?? "0");
  const agentId = process.env.HVAR_YIELD_SCOUT_AGENT_ID
    ? Number(process.env.HVAR_YIELD_SCOUT_AGENT_ID)
    : undefined;
  ...
  return {
    ...
    registry: {
      ...
      agentId,
      fallbackToAllowlist:
        process.env.HVAR_REGISTRY_FALLBACK_ALLOWLIST !== "false",
    },
  };
}
