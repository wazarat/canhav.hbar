  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      try {
        ...
        const repAddress = process.env.NEXT_PUBLIC_REPUTATION_REGISTRY_ADDRESS;
        ...
        console.warn("[.well-known/ucp] DB or registry error:", err);
      }
    }
    ...
    console.error("[.well-known/ucp]", error);
    return NextResponse.json(
      { error: "Failed to build UCP manifest" },
      { status: 500 }
    );
  }
}
