...
export function getHbarClient(): Client {
...
export async function createAuditTopic(): Promise<string> {
  ...
  const tx = new TopicCreateTransaction().setTopicMemo("hbar-agents-audit");
...
export function getStubWorkerId(): string {
  return process.env.HBAR_STUB_WORKER_ID || process.env.HEDERA_OPERATOR_ID || "0.0.0";
}

export function getYieldScoutWorkerId(): string {
  return (
    process.env.HBAR_YIELD_SCOUT_WORKER_ID ||
