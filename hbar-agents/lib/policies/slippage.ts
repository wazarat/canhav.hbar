...
import { WRITE_TOOLS, logPolicyEvent } from "../policy-state";
...
export function cacheSwapQuote(
  sessionId: string,
...
export class SlippagePolicy extends AbstractPolicy {
  ...
  protected shouldBlockPostParamsNormalization(
    ...
    const cached = getCachedSwapQuote(this.sessionId);
    const swapParams = extractSwapParams(params.rawParams);

    if (!cached) {
      return block(this.sessionId, this, method, "No fresh swap quote — fetch a quote before executing");
    }

    if (Date.now() - cached.quotedAt > QUOTE_TTL_MS) {
      return block(this.sessionId, this, method, "Quote is stale — fetch a fresh quote before executing");
    }
    ...
    if (
      ...
      return block(this.sessionId, this, method, "Swap parameters do not match the cached quote");
    }

    const maxSlippage = swapParams.slippageTolerance ?? cached.maxSlippagePct;
    if (
      cached.priceImpact != null &&
      cached.priceImpact > maxSlippage
    ) {
      return block(
        this.sessionId,
        this,
